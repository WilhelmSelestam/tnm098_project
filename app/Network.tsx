...
export default function StarWarsNetwork({
  ...
  useEffect(() => {
    ...
    createMarker("arrowhead-amber", "#f59e0b")
  ...
  useEffect(() => {
    ...
    // 1. Detect cycles using Tarjan's Strongly Connected Components
    ...
    if (showCircularRelationships) {
      ...
      function strongconnect(v: string | number) {
        ...
        if (lowlinks.get(v) === indices.get(v)) {
          ...
          if (scc.length > 1) {
            sccs.push(scc) // Group has > 1 node -> forms a cycle!
          }
        }
      }
      ...
      svg.selectAll("line").each((d: any) => {
        const sourceId = typeof d.source === "object" ? d.source.id : d.source
        const targetId = typeof d.target === "object" ? d.target.id : d.target
        // If an edge's source and target share the same SCC, it's a circular edge
        if (
          sccMap.has(sourceId) &&
          sccMap.has(targetId) &&
    ...
    // 2. Identify logic defying relationship directions
    const isWeirdRelationship = (d: any) => {
      ...
      for (const linkType of linkTypes) {
        ...
        if (linkType === "ownership" && sourceType === "vessel") {
          return true // A vessel can't own things
        }
      }
      return false
    }
    ...
    svg
      ...
      .attr("stroke", (d: any) => {
        ...
        if (showWeirdRelationships && isWeirdRelationship(d)) return "#f59e0b"
        return "#6666" // Base color
      })
      .attr("stroke-width", (d: any) => {
        const baseWidth = Math.max(1, d.count ?? 1)
        if (showCircularRelationships && cyclicLinks.has(d))
          return baseWidth + 2 // Make it thicker
        if (showWeirdRelationships && isWeirdRelationship(d))
          return baseWidth + 1
        return baseWidth
      })
      .attr("stroke-dasharray", (d: any) => {
        if (showWeirdRelationships && isWeirdRelationship(d)) return "5,5" // Dash it
        return "none"
      })
